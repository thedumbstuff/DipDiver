...
def _run_and_wait(client: TestClient, job_id: str, timeout: float = 15.0) -> str:
    """POST /triggers/run, then poll /triggers/status until the run finishes.

    Jobs run in a background thread now (the POST returns a polling fragment
    immediately), so tests must wait for completion before asserting on the
    JobLog row or job side-effects. Returns the final status fragment.
    """
    import re
    import time

    r = client.post("/triggers/run", data={"job_id": job_id})
    assert r.status_code == 200
    m = re.search(r"/triggers/status/(\d+)", r.text)
    assert m, f"expected a polling fragment, got: {r.text}"
    url = m.group(0)
...
def test_trigger_pnl_settle_returns_stub_ok(client: TestClient, data_root: Path):
    """The pnl_settle job is a stub — should finish with rc=0 cleanly."""
    body = _run_and_wait(client, "pnl_settle")
    assert "pill-ok" in body
    assert "rc=0" in body


def test_trigger_writes_joblog_row(client: TestClient, data_root: Path):
    _run_and_wait(client, "pnl_settle")
    from dipdiver.ui import db
...
def test_trigger_scoreboard_render_writes_file(
    client: TestClient, data_root: Path, seeded_scoreboard
):
    _run_and_wait(client, "scoreboard_render")
...
def test_failing_trigger_marks_joblog_error(
    ...
    body = _run_and_wait(client, "pnl_settle")
    assert "pill-err" in body or "rc=1" in body
